...
public class BrowserHandler {
    ...
    private void setOptionIfDefined(Object options, Properties properties, String propertyName) {
       ...
       //Setting capabilities according to their driver options
                setCapability(options, propertyName, boolValue);
            } else {
                String[] optionParts = optionValue.split("=");

                if (optionParts.length == 2) {
                    addArguments(options, optionParts[0] + "=" + optionParts[1]);
                } else {
                    addArguments(options, optionValue);
    ...
    private void setCapability(Object options, String capabilityName, boolean capabilityValue) {
        ...
        // Add conditions for other browser options if needed
    }
    
    private void addArguments(Object options, String optionValue) {
        if (options instanceof ChromeOptions) {
            ChromeOptions chromeOptions = (ChromeOptions) options;
            chromeOptions.addArguments(optionValue);
        } else if (options instanceof FirefoxOptions) {
            FirefoxOptions firefoxOptions = (FirefoxOptions) options;
            firefoxOptions.addArguments(optionValue);
        }
...
//        }
    ...
    private EdgeOptions getEdgeOptions() {
        ...
        for (String propertyName : edgeProperties.stringPropertyNames()) {
        	setOptionIfDefined(options, edgeProperties, propertyName);
        }
