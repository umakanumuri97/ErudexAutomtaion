...
public class LoginPage extends ErudexPage {
	...
	public ErudexPage login(boolean accept) {
		...
		try {
			...
			logger.info("Login Button Click Exception [" + ex.getClass().getName() + "]" + "[" + ex.getMessage() + "]");
			SessionUtils.getScreenShot(driver);
			WebElementUtils.jsClick(login, driver);
	...
	private boolean isInvalidLoginMessageDisplayed() {
		try {
			...
			wait.until(ExpectedConditions.textToBePresentInElementLocated(By.cssSelector("div.error-msg"),
	...
	public void setSessionOverRide(String overRide) {
		// Cast the driver to JavascriptExecutor
		JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;

		// Set the variable value using executeScript()
		String variableName = "overrideSession";
		String variableValue = overRide;
		jsExecutor.executeScript("window." + variableName + " = '" + variableValue + "';");
	}
}
